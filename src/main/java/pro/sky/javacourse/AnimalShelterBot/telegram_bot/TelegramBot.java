...
// End of code for replying incoming messages
    ...
    /** Method where comes updates from {@link TelegramBot#onUpdateReceived(Update)}
     * when {@link BotState#COMMON}. Active when user is in default {@link BotState}.
     * @param update generated by Telegram when message sent to bot.
     */
    ...
    /** Method where comes updates from {@link TelegramBot#onUpdateReceived(Update)}
     * when {@link BotState#VOLUNTEER_CHAT}. Active when user in chat with volunteer.
     * @param update generated by Telegram when message sent to bot.
     */
    ...
    /** Method where comes updates from {@link TelegramBot#onUpdateReceived(Update)}
     * when {@link BotState#COLLECT_DATA}. Active when user is in sending contact to shelter.
     * @param update generated by Telegram when message sent to bot.
     */
    ...
    /** Method where comes updates from {@link TelegramBot#onUpdateReceived(Update)}
     * when {@link BotState#REPORT}. Active when user is in creating and saving report.
     * Photos from unsaved reports are stored in files with <i>INCOMPLETE</i> suffix.
     * Old photos with <i>INCOMPLETE</i> suffix can be deleted safely.
     * @param update generated by Telegram when message sent to bot.
     */
...
// End of code for downloading photo from telegram update
    ...
    /**
     * Custom method for creating and sending text message. Created to make code shorter and more readable.
     * Used where it possible in another methods.
     * @param chatId  receiver chat identifier of type Long.
     * @param message String message that should be delivered to receiver.
     */
    ...
    /**
     * Method for checking is a user with given chatId a volunteer in shelter with given shelterId or not.
     * @param chatId  chat user identifier of type Long.
     * @param shelterId shelter identifier of type Long.
     */
    ...
    /**
     * Method for extracting Long id's of particular object ({@link Shelter}, {@link Pet}, {@link Report}) to operate
     * with. CallbackData also begins with a string which must not contain another digits not referred to id.
     * String part at the beginning of callbackData identifies the method that shall be executed with id parameter
     * passed to it.
     * @param callbackData String data extracted from update.
     */
    ...
    /**
     * Method for creating {@link InlineKeyboardButton} in one line of code.
     * @param name String that will be on the button.
     * @param callbackData String callbackData that will be returned with update after user activates a button.
     */
    ...
    /**
     * Method for execute telegram SendMessage message and handle {@link TelegramApiException} in a single line of code.
    ...
    /**
     * Method for edit text in message and execute telegram message while handling {@link TelegramApiException}.
...
// Retrieving particular menu / bot message
    /** Method for showing help message to user after {@code /help} command is executed.
     * Don't work in {@link BotState#REPORT} state, where the {@link TelegramBot#menuReportHelp(Long)}
    ...
    /** Method for showing help message to user in {@link BotState#REPORT} state, when user executes {@code /help} command
     * from report creating menu.
     * @param chatId chat id of user.
     */
    ...
    /**
     * Specific method for construct and send Shelter select menu, that consists of list of short messages
     * about each shelter inline buttons attached to each message.
     * This method uses custom methods:<br>{@link TelegramBot#createInlineKeyboardMarkup(String[][]...)}
     * to create buttons and {@link TelegramBot#executeAndSendMessage(SendMessage)} to send message.
     * This method also uses {@link TelegramBot#menuReportStart(Long)} method to show start report message,
     * if {@link TelegramBot#menuReportStart(Long)} decides to do so.
     ...
     * This method creates most detailed menu for selected shelter.
     * This method uses custom methods:<br>{@link TelegramBot#createInlineKeyboardMarkup(String[][]...)}
     * to create buttons and {@link TelegramBot#executeAndSendMessage(SendMessage)} to send message<br>
...
// добавить проверку есть ли в данном приюте непроверенные отчеты
    ...
    /**
     * Specific method that edits message of particular shelter from {@link TelegramBot#menuShelterSelect(Long chatId)}
     * menu to show roadmap of that shelter.
     * @param update of Update class transfers information like chatId, messageId, callbackData including shelterId.
     */
    ...
    /**
     * Specific method that shows to user message with basic information of how to get a pet from a shelter.
     * @param chatId  chat user identifier of type Long.
     * @param shelterId shelter identifier of type Long.
     */
    ...
    /**
     * Specific method that shows user a menu of animal types in shelter or passes user to select available pets
     * with {@link TelegramBot#menuPetSelect(Long chatId, Long shelterId, PetType)} method if there is only one type of animals
     * in current shelter.
     * @param chatId  chat user identifier of type Long.
     * @param shelterId shelter identifier of type Long.
     */
    ...
    /**
     * Specific method that shows user a menu of animal types in shelter or passes user to select available pets with
     * {@link TelegramBot#menuPetSelect(Long chatId, Long shelterId, PetType)} method if there is only one type of
     * animal in current shelter.
     * @param chatId  chat user identifier of type Long.
     * @param shelterId shelter identifier of type Long.
     */
...
//        if there is only one pet type then return button will guide to shelter menu, else to selectPetType menu.
    ...
    /**
     * Specific method that puts bot in {@link BotState#VOLUNTEER_CHAT} state and shows user a button to exit chat
     * with current shelters volunteer.
     * @param chatId  chat user identifier of type Long.
     * @param shelterId shelter identifier of type Long.
     */
    ...
    /**
     * Specific method shows or doesn't show a message with buttons that guides user to
     * {@link TelegramBot#menuReportPetSelect(Long chatId)} pet select menu for creating a report.
     * If caretaker has pets from different shelters on adoption, they all will be available.
     * @param chatId  chat user identifier of type Long.
     */
    ...
    /**
     * Method sends list of messages to select one for report creation on particular pet that caretaker has on adoption.
     * If caretaker has only one pet on adoption, then report creation menu
     * {@link TelegramBot#menuReportPetCurrent(Long chatId, Long petId)} is opens immediately.
     * @see TelegramBot#menuReportPetSelect(Long chatId, Long shelterId)
     * @param chatId  chat user identifier of type Long.
     */
    ...
    /**
     * Method sends list of messages to select a pet from current shelter for report creation.
     * If caretaker has only one pet on adoption, then report creation menu
     * {@link TelegramBot#menuReportPetCurrent(Long chatId, Long petId)} is opens immediately.
     * @see TelegramBot#menuReportPetSelect(Long chatId)
     * @param chatId  chat user identifier of type Long.
     * @param shelterId shelter identifier of type Long.
     */
    ...
    /**
     * Specific method that puts bot in {@link BotState#REPORT} state and shows user a buttons to save or
     * exit report about selected pet. Method uses {@link TelegramBot#tempReports} to store unsaved reports.
     * @param chatId  chat user identifier of type Long.
     * @param petId pet identifier of type Long.
     */
...
//        CREATING REPORT TO WORK WITH
    ...
    /**
     * Specific method puts bot in {@link BotState#COLLECT_DATA} state and shows user a buttons to send a contact
     * to shelter or exit that state.
     * Because {@link KeyboardButton button} sends a message to user, this method uses shelter names to
     * transfer information for which shelter contact was created. That means that <b>shelter names must be unique</b>.
     * If there are more then one shelter with the same name, consider adding more identification to
     * shelter name (e.g. city name or street name).
     * @param chatId  chat user identifier of type Long.
     * @param shelterId shelter identifier of type Long.
     */
    ...
    /**
     * Specific method shows a roadmap image of shelter.
     * @param update update created by telegram.
     */
    ...
    /**
     * Method shows information about pet and its avatar.
     * @param chatId chat user identifier of type Long.
     * @param petId pet identifier of type Long.
     */
    ...
    /**
     * Method that deletes report from {@link TelegramBot#tempReports}.
     * @param chatId chat user identifier of type Long.
     */
    ...
    /**
     * Method that creates menu messages with unchecked reports. So the volunteer can select which report to check.
     * Uses {@link TelegramBot#menuReportSmall(Long chatId, Report report)} for creating a particular message.
     * @param chatId chat user identifier of type Long.
     * @param shelterId shelter identifier of type Long.
     */
    ...
    /**
     * Method creates a particular report menu message for {@link TelegramBot#menuReportSelect(Long chatId, Long shelterId)},
     * one for each unverified report.
     * @param chatId chat user identifier of type Long.
     * @param report {@link Report Report report} from database.
     */
    ...
    /**
     * Method creates a menu  to check the report and shows a report from database for volunteer.
     * @param update update created by telegram.
     */
